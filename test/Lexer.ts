...
describe('Lexer', () => {
    ...
    it('should parse binary integer literals correctly', () => {
        ...
        assert(
            ...
            `expected '1' error, instead got '${result.errors.length}' errors`
        );
    ...
    it('should parse boolean literals correctly', () => {
        ...
        result.tokens.forEach((token, i) => {
            assert(
                ...
                `expected '${
                    keys[i]
                }' to result into 'token.type' of 'TokenType.BooleanLiteral', instead got '${
                    TokenType[token.type]
                }'`
            );
            assert(
            ...
            deepStrictEqual(
                ...
                `expected '${
                    keys[i]
                }' to result in a specific 'BooleanTypeInfo'`
            );
        });
    });
